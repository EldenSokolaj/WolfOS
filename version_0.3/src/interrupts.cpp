...
const uint8_t IDT_DESC_PRESENT = 0x80;
    ...
    void interruptManager::setIDTentry(uint8_t interruptNumber, uint32_t CodeSegment, void (*handler)(), uint8_t discriptorAccess, uint8_t discriptorType){
    ...
    for(uint8_t i = 255; i > 0; --i){
        setIDTentry(i, codeSegment, &interruptManager::IgnoreInterruptRequest, 0, IDT_INTERRUPT_GATE);
    }
    setIDTentry(0, codeSegment, &interruptManager::IgnoreInterruptRequest, 0, IDT_INTERRUPT_GATE);
